namespace ImplicitTreap {
  ...
  struct node {
    ...
    int count, prio;
    node *l, *r, *p;
    string debug(int index) { return "(" + to_string(index) + "," + to_string(val) + ")"; }
  };

  static int getCount(node* root) { return root ? root->count : 0; }
  ...
  void split(node* t, node* &l, node* &r, int index, int add = 0) {
    if (!t) return void(l = r = NULL);
    int cur_index = add + getCount(t->l);
    if (index <= cur_index) split(t->l, l, t->l, index, add), r = t;
    else split(t->r, t->r, r, index, add + 1 + getCount(t->l)), l = t;
    update(t);
  }
  ...
  node* get(node* t, int index, int add = 0) {
    ...
    if(index > cur_index) return get(t->r, index, add + 1 + getCount(t->l));
    return t;
  }
  ...
  void print(node* t) { int index = 0; print(t, index, true); }
};
