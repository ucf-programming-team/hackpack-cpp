...
struct Edge {
    ...
    Edge *rev, *prev;
    bool used, isBorder;
    Edge(P a, P b): id(0), a(a), b(b), ab(b-a), rev(NULL), prev(NULL), used(0), isBorder(0) { }
...
// O(v + m log m)
template<class P>
vector<vector<Edge<P>*>> extractPolygons(vector<P> &points, vector<vi> &adjs) {
    using Edge = Edge<P>;
    int n = sz(points), curEId = 0; // # of poly-poly edges; can keep global
    ...
    for (int i = 0; i < n; i++) {
        ...
        for (Edge *e : edges[i]) {
            if (e->used) continue;
            e->used = true;
            vector<Edge*> edgeList{e}; cur = e->b;
            while (true) {
                e = e->rev->prev;
                if (e->used) break;
                e->used = true;
