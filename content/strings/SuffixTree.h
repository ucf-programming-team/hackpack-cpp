...
struct SuffixTree {
    ...
    State update(State s) {
        int oldr = root;
        auto [endPoint, r] = split(State{s.id, s.l, s.r-1}, st[s.r]);
        while (!endPoint) {
            newNode(s.r, oo);
            nodes[r].child[st[s.r]] = nodes.back().id;
            if (oldr != root) {
                nodes[oldr].suffLink = r;
            }
            oldr = r;
            State canon = canonize(State{nodes[s.id].suffLink, s.l, s.r-1});
            tie(s.id, s.l) = {canon.id, canon.l};
            tie(endPoint, r) = split(State{s.id, s.l, s.r-1}, st[s.r]);
        }
        if (oldr != root) {
            nodes[oldr].suffLink = s.id;
        }
        return s;
    }
};
