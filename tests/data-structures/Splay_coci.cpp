...
struct node {
	...
	node(int val, node* l = 0, node* r = 0): ch{l, r}, val(val) { sum = val; }
};
...
ll getSum(node* x) {
	if (!x) return 0;
	return !x->wipe * x->sum + x->shift * cnt(x) + x->slope * (cnt(x) + 1) * cnt(x) / 2;
}
...
void prop(node* x) {
	...
	if (x->ch[1]) x->ch[1]->shift += x->slope * (cnt(x->ch[0]) + 1);
	x->wipe = false;
	x->shift = 0;
	x->slope = 0;
	update(x);
